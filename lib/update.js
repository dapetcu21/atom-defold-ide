...
const patchSelfFunctions = ['on_message', 'on_input', 'on_reload', 'update', 'init', 'final']
...
function patchOptions (options) {
  ...
  options.namedTypes = {
    gameObjectInstance: { type: 'table', fields: {} }
  }
  if (options.global && options.global.type === 'table' && options.global.fields) {
    const fields = options.global.fields
    patchSelfFunctions.forEach(fname => {
      const func = fields[fname]
      if (func.type !== 'function') { return }
      func.argTypes = func.argTypes || []
      func.argTypes[0] = func.argTypes[0] || { type: 'ref', name: 'gameObjectInstance' }
    })
  }
}
